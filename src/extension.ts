...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider("chatgpt-sidebar-view", provider),
  );

  // 🧠 Explain Code Command
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "chatgptAutoLaunch.explainCode",
      async () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "chatgptAutoLaunch.refactorCode",
      async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) return;
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "chatgptAutoLaunch.beautifyCode",
      async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) return;
        ...
        try {
          const formatted = await beautifyCode(
            code,
          ...
          vscode.window.showErrorMessage(
            "⚠️ Could not format code: " + err.message,
          );
        }
      },
    ),
